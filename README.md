...
general-purpose "replace malloc" work — sized. Pick a mode and stay in it:
`memento_free` is sized-only. Exact pointers go through
`memento_thread_heap_free(heap, ptr, size)`. Passing an exact pointer to
`memento_free` is not mixed-mode cleverness — it is a no-op in release
(the block leaks) and a debug failure.
...
mappings, so the owning span of any pointer is one `AND` away. Inside each
span sit 32 × 64 KiB *pages* (`MEMENTO_PAGE_SIZE`). Each page is one size
class; the page header (including the free list and `nused`) lives in the
same 64 KiB as the blocks, so the hit path does not dirty a span header
2 MiB away. In front of the active page is a per-class LIFO tcache
(`MEMENTO_TCACHE_DEPTH`, default 16) so churn is an indexed load off a hot
line. Spans stay single-class. There are 29 size classes: 16-byte
granularity up to 256 B, then steps of at most ~28% up to 8 KiB.

A page's `nused` counts blocks in user hands. When every page in a span is
empty, the span is parked for reuse. That's the reclamation design.

**Foreign frees** of small blocks CAS onto the *page's* `thread_free` stack
(same exchange-all drain, now sharded). Large/page-run frees still use the
heap-wide stack. Push is a CAS loop, drain is a single `atomic_exchange`,
and the drained batch is bucketed by size class so the
